 ...
 * If collectionName is provided, organize into collection folder
 */
export function generateLocalPath(article, collectionName = null) {
  // Generate clean filename from title
  const filename = titleToFilename(article.title);

  // If collection provided, organize into folder
  if (collectionName) {
    const folderName = collectionToFolderName(collectionName);
    return path.join(ARTICLES_DIR, folderName, filename);
  }
