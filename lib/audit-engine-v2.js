/**
 * AI Audit Engine V2
 * Uses Gemini 3 Flash with improved prompt for better accuracy
 */
...
/**
 * Run AI audit using Gemini 3 Flash
 */
async function runGeminiAudit(changelogEntry, fullDocumentation) {
  const apiKey = process.env.GOOGLE_AI_API_KEY || process.env.GEMINI_API_KEY;

  if (!apiKey) {
    throw new Error('GOOGLE_AI_API_KEY or GEMINI_API_KEY not set in environment');
  }
  ...
  console.log('Running AI audit with Gemini 2.0 Flash...');
  console.log(`  Context: ${Math.round(fullDocumentation.length / 4)} tokens (approx)`);
...
export async function runAudit(changelogEntry) {
  try {
    console.log(`\n=== Starting Audit V2 (Gemini 3 Flash) ===`);
    ...
    console.log(`  Loaded ${articleList.length} articles (${Math.round(fullDocumentation.length / 1000)}KB)`);

    // Run AI audit
    console.log('Running AI audit...');
    const auditResult = await runGeminiAudit(changelogEntry, fullDocumentation);
    ...
    const logFile = path.join(auditLogPath, `audit-v2-${timestamp}.json`);
