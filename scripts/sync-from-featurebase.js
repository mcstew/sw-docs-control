#!/usr/bin/env node
/**
 * Sync FROM Featurebase
 * Pull Featurebase articles and update local markdown files
 */
...
async function main() {
  console.log('🚀 Syncing FROM Featurebase...\n');
  ...
  const remoteResponse = await client.getArticles({ help_center_id: helpCenterId });
  ...
  for (const remoteArticle of remoteArticles) {
    try {
      console.log(`Processing: ${remoteArticle.title} (${remoteArticle.id})`);
      ...
      if (!localArticle) {
        ...
        const { article, filePath } = await createLocalArticle(remoteArticle);
      ...
      if (!lastSync) {
        ...
        await updateLocalArticle(localArticle.path, remoteArticle);
        ...
        results.pulled.push({ id: remoteArticle.id, title: remoteArticle.title });
        console.log('');
        continue;
      }
      ...
      if (remoteUpdatedAt <= lastSyncedAt) {
        ...
        results.skipped.push({ id: remoteArticle.id, title: remoteArticle.title });
        console.log('');
        continue;
      }
  ...
  if (results.pulled.length > 0) {
    console.log('✅ Successfully pulled:');
    results.pulled.forEach(p => {
      console.log(`  - ${p.title}`);
    });
    console.log('');
  }
