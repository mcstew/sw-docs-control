# Docs Control Dashboard - Implementation Guide

This guide contains the essential code and assets to implement the "Prius Energy Monitor" inspired Docs AI orchestration dashboard.

## 1. Dependencies
Ensure the following packages are installed in your React project:
- `lucide-react` (for the UI icons)
- `tailwindcss` (The design relies heavily on Tailwind utility classes)
...
Add the following custom animations and utility classes to your global CSS file. This handles the CRT scanline effect, text glow, and the flowing energy lines.
...
This is the core React component. It uses absolute positioning to perfectly align the SVG flow lines with the HTML nodes.

```tsx
import React, { useState } from 'react';
import { Database, GitBranch, Sparkles, RefreshCw, FileSearch, Activity } from 'lucide-react';
...
## Key Technical Details for the Engineer

1. **SVG Flow Lines:** The "energy" flow is achieved using SVG `<path>` elements with `stroke-dasharray="12 12"`. The CSS animations (`flow-forward`, `flow-backward`) animate the `stroke-dashoffset` property to create the illusion of movement.
2. **Responsive Alignment:** The nodes and SVG paths use percentage-based positioning (e.g., `left-[20%]`, `left-[50%]`) within a `relative` container. This ensures the lines always connect exactly to the centers of the nodes, regardless of the screen size.
3. **Z-Indexing & Overlays:** The `.scanlines` overlay has `pointer-events-none` and sits above the background but below the text/nodes to create the screen effect without blocking clicks.
4. **State-Driven Animation:** The `className` on the SVG paths dynamically switches between `.animate-flow-slow` and `.animate-flow-fast` based on the React state (`isSyncing`, `isAuditing`) to give immediate visual feedback.
